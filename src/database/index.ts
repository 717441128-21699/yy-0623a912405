...
class JsonDatabase {
  ...
  private initializeDefaultRules() {
    ...
    this.data.alertRules = [
      ...
      { id: 'rule-critical-power-high', name: '断电重大（高敏感）', alertType: 'external_power_disconnect', minPowerOffMinutes: 10, goodsSensitivityRequired: 'high', nearDeliveryRequired: false, targetLevel: 'critical', enabled: true, createdAt: now, updatedAt: now },
      { id: 'rule-critical-power-long', name: '断电重大（长时间）', alertType: 'external_power_disconnect', minPowerOffMinutes: 30, goodsSensitivityRequired: null, nearDeliveryRequired: false, targetLevel: 'critical', enabled: true, createdAt: now, updatedAt: now },
      ...
      { id: 'rule-urgent-battery', name: '电压异常紧急', alertType: 'battery_voltage_abnormal', minPowerOffMinutes: 5, goodsSensitivityRequired: 'high', nearDeliveryRequired: false, targetLevel: 'urgent', enabled: true, createdAt: now, updatedAt: now },
      ...
      { id: 'rule-urgent-refrigeration', name: '冷机停报紧急', alertType: 'refrigeration_stop_reporting', minPowerOffMinutes: 15, goodsSensitivityRequired: 'medium', nearDeliveryRequired: false, targetLevel: 'urgent', enabled: true, createdAt: now, updatedAt: now },
      { id: 'rule-critical-refrigeration', name: '冷机停报重大', alertType: 'refrigeration_stop_reporting', minPowerOffMinutes: 30, goodsSensitivityRequired: 'high', nearDeliveryRequired: false, targetLevel: 'critical', enabled: true, createdAt: now, updatedAt: now }
    ];
    this.scheduleWrite();
  }
